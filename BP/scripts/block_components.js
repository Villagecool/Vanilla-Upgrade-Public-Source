...
import { setPermutation, getRandomInt, offsetLocation, setVectorFloats, decripateStack, vec3toString, lerp, getRandomBool } from './utils.js'
...
SERVER.world.beforeEvents.worldInitialize.subscribe(initEvent => {
    ...
    initEvent.blockComponentRegistry.registerCustomComponent('vc:sapling', {
        onRandomTick: e => {
            ...
            if (getRandomInt(0, 5) == 0) e.block.dimension.runCommand(`structure load ${e.block.typeId.replace('vc:', '').replace('_sapling', '')}_tree1 ${e.block.location.x - 3} ${e.block.location.y} ${e.block.location.z - 3}`)
        },
        onPlayerInteract: e => {
    ...
    initEvent.blockComponentRegistry.registerCustomComponent('vc:glorium_crystal_block_budding', {
        onRandomTick: e => {
            ...
            if (!victimBlock.typeId.startsWith('vc:glorium_crystals')) return;
            if (victimBlock.permutation.getState("minecraft:block_face") == dirfaces[chosenFace]) {
                var face = victimBlock.permutation.getState("minecraft:block_face")
                victimBlock.setType(
                    (victimBlock.typeId == 'vc:glorium_crystals_tiny' ? 'vc:glorium_crystals_small' :
                        victimBlock.typeId == 'vc:glorium_crystals_small' ? 'vc:glorium_crystals_medium' :
                            victimBlock.typeId == 'vc:glorium_crystals_medium' ? 'vc:glorium_crystals' : 
                                victimBlock.typeId == 'minecraft:air' ? 'vc:glorium_crystals_tiny' : undefined
                    ))
                if (victimBlock) setPermutation(victimBlock, 'minecraft:block_face', face)
            }
        }
    });
    ...
    initEvent.blockComponentRegistry.registerCustomComponent('vc:crop', {
        onRandomTick: e => {
            var maxGrowth = (e.block.typeId == 'vc:blue_berries' || e.block.typeId == 'vc:warped_wart' ? 3 : e.block.typeId == 'vc:aloe_plant' ? 2 : 6)
            setPermutation(e.block, 'vc:growth', (e.block.permutation.getState("vc:growth") + 1).clamp(0, maxGrowth))
        },
        onPlayerInteract: e => {
            var maxGrowth = (e.block.typeId == 'vc:blue_berryies' || e.block.typeId == 'vc:warped_wart' ? 3 : e.block.typeId == 'vc:aloe_plant' ? 2 : 6)
    ...
    initEvent.blockComponentRegistry.registerCustomComponent('vc:cake', {
        onPlayerInteract: e => {
            ...
            e.player.eatItem(new SERVER.ItemStack('vc:cake_food_stats', 1))
        }
    });
    ...
    initEvent.blockComponentRegistry.registerCustomComponent('vc:melon_golem_spawn', {
        beforeOnPlayerPlace: e => {
            if (e.block.below(1).typeId == 'minecraft:snow' && e.block.below(2).typeId == 'minecraft:snow') {
                ...
                SERVER.system.runTimeout(() => { e.block.setType('minecraft:air') }, 1)
                e.dimension.spawnEntity('vc:melon_golem', e.block.below(2).center())
            }
        }
    });
    ...
    initEvent.blockComponentRegistry.registerCustomComponent('vc:rope', {
        onPlayerInteract: e => {
            ...
            if (pitem == 'vc:rope') {
                ...
                const rope = e.player.getComponent('minecraft:riding')?.entityRidingOn || spawnRopeThingy(e.player)
                if (rope.nameTag == 'no') return;
                var ropeLoc = rope.dimension.getBlock(rope.location).location
                //rope.applyKnockback(0,0,0,(e.block.location.y - ropeLoc.y)*0.01)
                const theRopeDist = e.block.location.y - ropeLoc.y - 1
                rope.nameTag = 'no'
                ...
                function spawnRopeThingy(entity) {
                    var theDude = e.dimension.spawnEntity('vc:climb_rope', { x: e.block.center().x, y: entity.location.y, z: e.block.center().z })
        ...
        onPlayerDestroy: e => {
            ...
            while (bottomBlock.below(1).typeId == 'vc:rope') {
...
function getrstrength(block) {
    if (block.getRedstonePower() != undefined) return block.getRedstonePower()
    let rstrengh = [];
...
SERVER.Block.prototype.vcIsSolid = function () {
    return this.isSolid ||
