...
SERVER.world.beforeEvents.playerBreakBlock.subscribe((data) => {
  ...
  if (badOmen != undefined) {
    SERVER.system.run(() => {
      ...
      player.runCommand(
        `particle vc:giant_omen_particle ${block.location.x} ${block.location.y} ${block.location.z}`
      );
      player.runCommand(
        `playsound effects.giant_omen @s ${block.location.x} ${block.location.y}.5 ${block.location.z}`
      );
    });
  }
});
SERVER.system.afterEvents.scriptEventReceive.subscribe((data) => {
  ...
  if (data.id === "vc:item_fix") {
    ...
    for (let i = 0; i < container.size; i++) {
      ...
      if (item != undefined) {
        if (item.typeId.startsWith("vc:") && item.typeId.endsWith("_door")) {
  ...
  if (data.id === "vc:xp_bomb") {
    ...
    for (let i = 0; i < 200; i++) {
      let xp = entity.dimension.spawnEntity("minecraft:xp_orb", {
        x: entity.location.x,
        y: entity.location.y + 8,
        z: entity.location.z,
      });
      if (i % 4 != 0) continue;
      let flesh = entity.dimension.spawnItem(
        new SERVER.ItemStack("minecraft:rotten_flesh", 1),
        { x: entity.location.x, y: entity.location.y + 8, z: entity.location.z }
      );
  ...
  if (data.id === "vc:effectCallback" && data.message === "giant_omen") {
    ...
    let BIGBOI = entity.dimension.spawnEntity("vc:giant", {
      x: entity.location.x,
      y: entity.location.y,
      z: entity.location.z,
    });
